...
interface ITripRepository extends IRepository<Trip, CreateTripInput, UpdateTripInput> {
  findByFilter(filters: TripFilter): Promise<Trip[]>;
}

interface ICostRepository extends IRepository<Cost, CreateCostInput, UpdateCostInput> {
...
interface IWorkSessionRepository extends IRepository<WorkSession, CreateWorkSessionInput> {
  findActive(): Promise<WorkSession | null>;  // the open session (endTime IS NULL)
}

interface IGoalRepository extends IRepository<Goal, CreateGoalInput, UpdateGoalInput> {}
...
type UpdateTripInput = Partial<Omit<CreateTripInput, 'platform'>> & { platform?: string };
```
