...
describe('Update use-cases branch coverage', () => {
  it('UpdatePlatformProfitGoal: validation, not found, success', async () => {
    ...
    await expect(uc.execute('id', { targetAmount: -1 } as any)).rejects.toThrow(
      ValidationError
    )

    const notFoundRepo: any = { findById: async () => null }
    const uc2 = new UpdatePlatformProfitGoal(notFoundRepo)
    await expect(uc2.execute('id', { targetAmount: 10 })).rejects.toThrow(
      NotFoundError
    )
  ...
  it('UpdateProfit: validation, not found, success', async () => {
    ...
    await expect(uc.execute('id', { amount: -1 } as any)).rejects.toThrow(
      ValidationError
    )
    const uc2 = new UpdateProfit({ findById: async () => null } as any)
    await expect(uc2.execute('id', { amount: 10 })).rejects.toThrow(
      NotFoundError
    )
  ...
  it('UpdateReminder: not found and update path', async () => {
    ...
    const existing = {
      ...
      date: new Date(),
      alarm: true,
      update: (input: any) => ({ ...existing, ...input }),
    }
    const repo: any = {
