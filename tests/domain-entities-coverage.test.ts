...
describe('Domain entities branch coverage', () => {
  ...
  it('PlannedAbsence.update throws on invalid input', () => {
    const p = PlannedAbsence.reconstitute({ id: 'p1', type: 'vacation', date: new Date('2025-01-01'), endDate: new Date('2025-01-10') })
    // invalid workedDays type should fail validation
    expect(() => p.update({ workedDays: ['not-a-date'] as any })).toThrow(ValidationError)
  })

  it('Reminder.update validation branch and recurrence/null handling', () => {
    const r = Reminder.reconstitute({ id: 'r1', message: 'm', date: new Date('2025-01-01'), alarm: true })
  ...
  it('Vehicle displayName and update color/notes branches', () => {
    const v = Vehicle.reconstitute({ id: 'v1', name: 'MyCar', plate: '   ', brand: 'B', model: 'M', year: 2020, fuelTypeId: 'f', typeId: 't' })
  ...
  it('Maintenance.update branches for title/estimatedCost/trigger', () => {
    ...
    const upd = m.update({ title: 'new', estimatedCost: 15, trigger: { type: 'mileage', mileage: 123 } as any })
  ...
  it('Reminder.update branches for message/date/alarm/recurrence', () => {
    const r = Reminder.reconstitute({ id: 'r2', message: 'm', date: new Date('2025-01-01'), alarm: true })
    const rec = { rule: 'FREQ=DAILY', endDate: new Date('2025-12-31') }
    const upd = r.update({ message: 'hello', date: new Date('2025-02-02'), alarm: false, recurrence: rec })
  ...
  it('Reminder.update clears recurrence when set to null', () => {
    const r = Reminder.reconstitute({ id: 'r3', message: 'm', date: new Date('2025-01-01'), alarm: true, recurrence: { rule: 'FREQ=DAILY' } })
